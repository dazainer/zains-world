...
async function ensureTable(sql: SqlClient) {
  await sql`
    CREATE TABLE IF NOT EXISTS tictactoe_leaderboard_entries_v2 (
  ...
  `
  await sql`
    CREATE INDEX IF NOT EXISTS tictactoe_leaderboard_entries_v2_score_idx
    ON tictactoe_leaderboard_entries_v2 (difficulty, score DESC, updated_at ASC)
  `

  const legacyRows = await sql`
    SELECT to_regclass('public.tictactoe_leaderboard_entries') AS legacy
  `
  ...
  await sql`
    INSERT INTO tictactoe_leaderboard_entries_v2 (
      username,
      username_normalized,
      difficulty,
      score,
    ...
    SELECT
      ...
      username_normalized,
      CASE
        WHEN LOWER(COALESCE(metadata, '')) IN ('easy', 'medium', 'hard') THEN LOWER(metadata)
...
async function handleGet(req: VercelRequest, res: VercelResponse) {
  ...
  const rows = await sql`
    ...
    FROM tictactoe_leaderboard_entries_v2
    WHERE score > 0
...
async function handleSubmit(body: SubmitRequest, res: VercelResponse) {
  ...
  if (!isOverrideSubmission) {
    const claimedRows = await sql`
      SELECT username_normalized FROM tictactoe_leaderboard_entries_v2
  ...
  if (existingRows.length > 0) {
    ...
    const topRows = await sql`
      SELECT score FROM tictactoe_leaderboard_entries_v2
  ...
  await sql`
    INSERT INTO tictactoe_leaderboard_entries_v2 (username, username_normalized, difficulty, score, metadata, owner_session_id)
    VALUES (
      ${identity.display},
      ${identity.normalized},
      ${difficulty},
