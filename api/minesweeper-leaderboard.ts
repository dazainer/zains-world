...
async function handleSubmit(body: SubmitRequest, res: VercelResponse) {
  ...
  const existingRows = await sql`
    SELECT score, owner_session_id FROM minesweeper_leaderboard_entries
  ...
  if (!isOverrideSubmission) {
    const claimedRows = await sql`
      SELECT username_normalized FROM minesweeper_leaderboard_entries
  ...
  await sql`
    INSERT INTO minesweeper_leaderboard_entries (username, username_normalized, score, metadata, owner_session_id)
